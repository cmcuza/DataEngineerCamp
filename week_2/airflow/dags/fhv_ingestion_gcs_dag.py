...
with workflow as dag:
    ...
    remove_csv_from_docker = BashOperator(
        task_id="rm_file_task",
        bash_command=f"rm {path_to_local_home}/{dataset_file}"
    )

    remove_parquet_from_docker = PythonOperator(
        task_id="rm_parquet_task",
        python_callable=rm_parquet,
        op_kwargs={
            "src_file": f"{path_to_local_home}/{dataset_file}",
        },
    )
    ...
    download_dataset_task  >> format_to_parquet_task >> local_to_gcs_task >> remove_csv_from_docker >> remove_parquet_from_docker